...
The matrix below records focused evidence for the original 11 acceptance scenarios. The post-review correction evidence at the end records the additional durable-attempt, bounded-observation, locking/durability, archive-ancestry, immutable-metadata, and cleanup-pending regressions; this archived evidence is historical and not a current synchronization gate.
...
| Invariant | Evidence |
|---|---|
| No caller-provided GitHub observation | `observeMergedPullRequest` (`candidate-receipt-retirement-remote.ts:53-86`) is the only path that builds a `NormalizedMergedPullRequestObservation`, and it parses the live `gh pr view --repo <repository> --json ...` output. Tool wiring in `ein-ai.ts:1396-1440` supplies its result to the decision; `evaluateCandidateReceiptRetirement` (`delivery-receipt.ts:157`) rejects `observation: undefined`. |
...
`tasks.md` marks groups 001 through 004 complete. The original acceptance scenarios (`candidate-receipt-bound-retirement-evidence`, `candidate-receipt-invalid-attempt-or-receipt`, `candidate-receipt-attempt-rotation`, `candidate-receipt-terminal-boundary`, `candidate-receipt-successful-retirement`, `candidate-receipt-archive-before-deactivate`, `candidate-receipt-idempotent-retirement`, `candidate-receipt-retirement-concurrent-revalidation`, `candidate-receipt-explicit-retirement-trigger`, `candidate-receipt-fresh-network-truth`, `candidate-receipt-mechanical-overlap-lifecycle`) map to passing tests in the matrix above. The later post-review scenarios are documented by the historical correction evidence below, not by a current OpenSpec synchronization status.
...
- `ein-pi/agent/lib/candidate-receipt-retirement-remote.ts` — `normalizeGitHubRepository`, `resolveExplicitPushRemoteRepository`, `observeMergedPullRequest`, timeout- and AbortSignal-aware command adapter.
- `ein-pi/agent/extensions/ein-ai.ts` — `RetirementToolParams`, `pi.registerTool("ein_candidate_receipt_retire", ...)`, remote-adapter wiring, and attempt cleanup after `retired`.
...
- `bun test tests/candidate-receipt.test.ts tests/delivery-gate.test.ts tests/candidate-receipt-retirement-remote.test.ts tests/candidate-receipt-retirement-tool.test.ts` — passed: 115 tests, 272 assertions, 0 failures. The new seams observe every new archive directory and its parent fsynced before the active unlink, and force a competing `retirement.json` write between inspection and exclusive link to prove that conflicting bytes preserve the active receipt.
- `bun test` — passed: 938 tests, 2,573 assertions, 0 failures across 83 files.
- `bun run --cwd installer typecheck` — passed: `tsc --noEmit` completed without diagnostics.
- `git diff --check` — passed: no whitespace errors.

The regenerated historical sync report is intentionally `conflict`: it is calculated from the current archived delta and current canonical spec, and records every archived `ADDED` scenario that already exists as `added-existing`. The three post-review scenarios—durable attempt recovery, unique push URL plus bounded `gh` observation, and owner-matched lifecycle locking with directory durability—are documented in both places without claiming a second clean application. This is historical evidence, not an active synchronization gate; it does not claim a clean completed sync.
