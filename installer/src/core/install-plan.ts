...
function piEntries(input: InstallPlanInput): ManagedInstallEntry[] {
  ...
  return [
    ...
    { id: "pi.deploy-template", runtime: "pi", action: "deploy", state: "selected", destination: input.piAgentDir, ownership, reason: `deploy managed Pi template in ${input.flags.skipLinear ? "solo" : "team"} mode and preserve user state` },
