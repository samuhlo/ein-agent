...
- **Review Workload Guard.** You do NOT run `git diff` yourself. Use `ein_review_forecast` for a preview when useful. After commit, delegate with the PR base and branch; `ein-git` runs `review-current` before push and PR. If over budget, the human chooses one PR or a split via `ask_user_question`. After an explicit single-PR choice, call `ein_review_exception` once to bind that session decision to the committed diff; do not ask again. Unknown or changed measurements block. Include accepted exception metrics in the PR body. Tests and density are informational; `auto` does not bypass the budget.
