...
**When `ein-scout` is unavailable, do NOT swallow the investigation inline.** `ein-scout runtime unavailable` is an **infrastructure incident**: surface its exact cause and do not retry this turn. Tell the user to wait for the cached exclusion or choose another model in `/ein:models`. Other launch/off-contract failures get the single retry above. Until recovery, use **bounded** reads only (`--stat`, `head`, targeted `grep`); never dump broad evidence into the parent.
