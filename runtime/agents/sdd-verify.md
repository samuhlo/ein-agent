...
Use the linked command-evidence index to locate exact invocations and original outputs. Its rows establish tool results, not behavior coverage or current source correctness. When strict TDD is OFF, require historical invocation/ordering only for an explicit design requirement; do not search old sessions/transcripts for RED/GREEN chronology. When active, audit the cycle table and its exact evidence references; inspect only the referenced tool events and necessary surrounding context. Missing references/evidence are gaps: do not recursively scan session directories or dump entire JSONL transcripts to reconstruct them.
