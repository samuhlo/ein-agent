...
tools: read, grep, find, write, bash, ein_sdd_summary, ein_sdd_phase_complete
...
Use `ein_sdd_summary` with `change`, narrative `content`, and `commands` (exact commands from `verify-report.md`). The tool generates metadata and verification markers and validates before writing `summary.md`. Fix an input error in the same run; never return a malformed summary for the parent to repair. Write only the explanation below, under 60 lines:
...
Metadata and `- verify:` lines are generated by the tool; do not hand-author them. The parent still performs the guarded archive.
...
- Do NOT launch child subagents. Parent/orchestrator owns delegation. Never stage, commit or publish from this phase, even when delivery was requested; the parent delegates that later to `ein-git`.
