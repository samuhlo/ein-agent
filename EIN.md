...
- **TDD estricto** (`openspec/config.yaml: strict_tdd: true`) es el DEFAULT del proyecto, no una obligación por cambio: cada cambio declara su postura en `openspec/changes/<change>/preflight.json` y esa declaración manda sobre el config. La puerta es `bun test` desde la raíz. **Hay DOS typechecks y CI corre los dos**: `bun run typecheck` desde la raíz (cubre `ein-pi/` y `cc-ein/`) y `cd installer && bun run typecheck`. `bun test` en verde no basta: Bun no comprueba tipos.
