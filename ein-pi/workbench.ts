...
import { projectProjectState } from "./agent/lib/project-state.ts";
import {
  buildLaunchPlan,
...
export async function runWorkbenchEntrypoint(options: EntrypointOptions): Promise<number> {
...
function productionDependencies(candidates: readonly string[]): WorkbenchDependencies & { dispose: () => void } {
...
if (import.meta.main) {
  process.exitCode = await runWorkbenchEntrypoint({
    argv: Bun.argv.slice(2), cwd: process.cwd(), stdinTTY: Boolean(stdin.isTTY), stdoutTTY: Boolean(stdout.isTTY),
