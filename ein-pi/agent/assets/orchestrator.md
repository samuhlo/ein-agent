...
- **ein-git — normal delivery**: pass the exact delivery step (`commit these files…`, `open a PR for X, base main`). It must not run tests/builds or read the whole diff. Pass **`maxRuntimeMs: 300000`** — never the chain budget (`1800000`), but never the old 2-minute cap either: measured over 63 delivery runs it killed 7 of them mid-flight at 6-9 turns, and a delivery interrupted between `push` and `gh pr create` costs far more to reconcile than a slow `gh` costs to wait for.
...
3. `ein-git` with `context: "fresh"` and `maxRuntimeMs: 300000` for normal delivery itself.
