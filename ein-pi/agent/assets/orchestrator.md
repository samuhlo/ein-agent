...
**Acceptance & turn-budget are auto-injected — pass NEITHER.** `pi-subagents` infers an acceptance level when you omit it and demands a code-shaped `acceptance-report` that planning and non-behavioral phases can't honestly produce → a false `rejected`. A deterministic `tool_call` hook now injects `acceptance: { level: "none" }` (and, for `sdd-apply`, a `turnBudget` backstop) for every planning phase (`sdd-scope`/`sdd-map`/`sdd-design`/`sdd-tasks`/`sdd-close`) and non-behavioral `sdd-apply` — so a forgotten field never false-rejects. `ein_sdd_check` is the real gate; leave `sdd-verify` on auto (re-running the suite in the acceptance layer buys nothing). **NEVER route the SDD loop by the acceptance verdict — route by `ein_sdd_status` + `ein_sdd_check`.** Override only with an explicit `acceptance: { level: "verified", verify: [...] }` to force a per-group runtime re-run when the group adds tests.
