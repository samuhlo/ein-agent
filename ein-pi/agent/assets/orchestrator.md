...
- **Code/verification phases** (`sdd-apply`, `sdd-verify`): leave `acceptance` on auto (omit it). The inferred evidence contract fits real implementation work; a `rejected` there is signal, not noise — read the failed check and apply the subagent-retry rule (once, with the concrete gap named).
