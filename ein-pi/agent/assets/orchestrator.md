...
**Execution mode.** `interactive` (default): between phases show the concise result, state the next phase, ask to continue. `auto`: run back-to-back. Both the execution mode AND the Strict TDD choice (off/strict/auto) are asked ONCE, up front, in the preflight — so a user who picks `auto` + a TDD stance is not interrupted again mid-flow.
