...
**Strict TDD forwarding.** The preflight now asks the TDD stance up front (off/strict/auto) and that decision overrides `openspec/config.yaml` (OFF → standard, no RED/GREEN; ON → strict; AUTO → follow config). An explicit off/strict choice fixes the session override, so the mid-flow TDD ask gate no longer fires. When strict applies to a delegated apply, include in that single prompt: `STRICT TDD MODE IS ACTIVE. Test runner: <command>. Follow RED, GREEN, TRIANGULATE, REFACTOR. Record evidence.` In the fallback `ein-sdd` chain keep the shared `{task}` **phase-neutral** — no TDD line, or the read-only phases would run tests; there the decision reaches `sdd-apply` through the injected preflight block.
