...
**Phase result envelope (compact by contract):** `status, executive_summary, artifacts, next_recommended, risks, skill_resolution`. Each phase's envelope is copied VERBATIM into THIS context and it is never reset across a flow, so a fat envelope per phase is what fills you — the phase agents cap it deliberately (`executive_summary` ≤ 3 lines, `risks` ≤ 3 bullets, NO pasted artifact content / file lists / test tables / command output). The full detail is on disk in the phase's artifact; when you need it, **read that artifact** (a bounded read), do not ask the phase to inline it. If a returned envelope is verbose, that is the executor breaking contract — route on the compact fields, don't propagate the bloat.
