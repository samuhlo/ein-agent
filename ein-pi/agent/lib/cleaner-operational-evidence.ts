...
const MAX_MODEL_PACKET_BYTES = 384 * 1024;
...
export function cleanerEvidenceForModel(passive: CleanerPassiveEvidence, active?: CleanerActiveEvidence): string {
	const summary = JSON.parse(compactCleanerEvidence(passive, active)) as unknown;
	const packet = JSON.stringify({
		summary,
		admittedSource: passive.audit.files.map(({ path, source }) => ({ path, source })),
	});
	if (Buffer.byteLength(packet) > MAX_MODEL_PACKET_BYTES) throw new Error("Cleaner model evidence exceeds byte cap");
	return packet;
}
