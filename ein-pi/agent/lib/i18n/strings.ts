...
const EN: Record<string, string> = {
	...
	"cmd.sdd-next.description": "Show the next recommended SDD step for a named change without executing it",
	"cmd.sdd-audit.description": "Validate a change (all phases) or lint a design.md path",
	"cmd.sdd-close.description": "Close a verified change: move openspec/changes/<x> to archive/",
	"cmd.sdd-archive.description": "[legacy] Use /ein:sdd-close",
	...
	"cmd.models.full.description": "Full preset: orchestrator + sdd-design → gpt-5.5, apply → MiniMax-M3, rest → MiniMax-M2.7",
	...
	"models.preset.full.desc": "Orch + sdd-design → gpt-5.5  |  apply → M3  |  tasks/rest → M2.7",
	...
	"help.short": [
		...
		"- /ein:models:full  → preset gpt-5.5 (orchestrator + sdd-design), tasks/rest on M2.7",
		"- /ein:models:lite  → preset MiniMax-M3 (orch + design/apply), tasks/rest on M2.7",
		...
		"- /ein:sdd-next <change> [--auto] → show the next recommended step (dry-run; no phase execution)",
		"- /ein:sdd-close [change] → close a verified change: move it to archive/",
		"- /ein:sdd-check [change | path] → [legacy alias of /ein:sdd-audit]",
		"- /ein:sdd-archive [change] → [legacy alias of /ein:sdd-close]",
	...
	"help.full": [
		...
		"- /ein:sdd-next <change> [--auto] → manual read-only dispatcher: shows the next recommended step; --auto is dry-run today.",
		"- /ein:sdd-close [change] → close a verified change: move openspec/changes/<x> to archive/.",
		"- /ein:sdd-check [change | path] → [legacy alias of /ein:sdd-audit].",
		"- /ein:sdd-archive [change] → [legacy alias of /ein:sdd-close].",
...
const ES: Record<string, string> = {
	...
	"cmd.sdd-next.description": "Mostrar el siguiente paso SDD recomendado para un cambio nombrado sin ejecutarlo",
	"cmd.sdd-audit.description": "Validar un cambio (todas las fases) o lintar un path de design.md",
	"cmd.sdd-close.description": "Cerrar un cambio verificado: mueve openspec/changes/<x> a archive/",
	"cmd.sdd-archive.description": "[legacy] Usa /ein:sdd-close",
	...
	"help.short": [
		...
		"- /ein:sdd-next <cambio> [--auto] → muestra el siguiente paso recomendado (dry-run; no ejecuta fases)",
		"- /ein:sdd-close [cambio] → cerrar un cambio verificado: moverlo a archive/",
		"- /ein:sdd-check [cambio | ruta] → [alias legacy de /ein:sdd-audit]",
		"- /ein:sdd-archive [cambio] → [alias legacy de /ein:sdd-close]",
	...
	"help.full": [
		...
		"- /ein:sdd-next <cambio> [--auto] → dispatcher manual read-only: muestra el siguiente paso; --auto hoy es dry-run.",
		"- /ein:sdd-close [cambio] → cerrar un cambio verificado: mueve openspec/changes/<x> a archive/.",
		"- /ein:sdd-check [cambio | ruta] → [alias legacy de /ein:sdd-audit].",
		"- /ein:sdd-archive [cambio] → [alias legacy de /ein:sdd-close].",
