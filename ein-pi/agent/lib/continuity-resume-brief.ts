...
function payload(checkpoint: ContinuityCheckpoint, target: ProjectRuntimeProvider, warnings: readonly ContinuityReadinessWarning[], lists: {
	...
	return escapedJson({
		...
		...(checkpoint.version === 2 && checkpoint.objectiveEvidence.kind === "intent-draft" ? { intentDraftPath: `.ein/intent-drafts/${checkpoint.objectiveEvidence.work}.json` } : {}),
...
function frame(data: string, target: ProjectRuntimeProvider, revision: string, omissions: ContinuityResumeBriefOmissions): {
	...
	const content = [
		...
		"When objectiveEvidence.kind is intent-draft, read intentDraftPath before continuing. Preserve response IDs and their sources; ask only unresolved decisions. Never replay a running evidence experiment without recovering its recorded result.",
