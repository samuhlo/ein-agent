...
import { type McpCard, type McpCardResult } from "./mcp-card.ts";
...
export function installToolCardBridge(prototype: object, owner: CardOwner): (() => void) | undefined {
  ...
  if (!bridge) {
    const owners = new Set<CardOwner>();
      ...
      return (args, theme, context) => {
        ...
        const component: Component = {
          ...
          render(width) {
            ...
            return selected.render({ tool, args, partial: true, started: context.executionStarted, error: false, expanded: context.expanded, expandHint: keyText("app.tools.expand") }, width, theme);
          },
        };
        return component;
      };
    ...
    const result: Host["getResultRenderer"] = function (this: Host) {
      ...
      return (output, options, theme, context) => {
        ...
        return {
          invalidate() {},
          render(width) {
            return selected.render({ tool, args: context.args, result: output, partial: options.isPartial, started: context.executionStarted, error: context.isError, expanded: options.expanded, durationMs: selected.duration(context.toolCallId), expandHint: keyText("app.tools.expand") }, width, theme);
          },
        };
      };
    };
