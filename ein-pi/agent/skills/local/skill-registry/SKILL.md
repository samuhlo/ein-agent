...
- Always write `.pi/ein/atl/skill-registry.md` regardless of SDD persistence mode.
...
- Add `.pi/ein/` to `.gitignore` when possible unless explicitly disabled.
...
3. Render `.pi/ein/atl/skill-registry.md` with scanned sources, registry contract, skill name, trigger/description, scope, and exact path.
...
Return:
- Project name and `.pi/ein/atl/skill-registry.md` path.
