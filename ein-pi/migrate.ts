#!/usr/bin/env bun
// =============================================================================
// ein-pi MIGRATE — mueve la instalación EIN de ~/.pi/agent a ~/.pi-ein/agent
// -----------------------------------------------------------------------------
...
console.log(`\nein-pi migrate: ~/.pi/agent → ~/.pi-ein/agent${dryRun ? "  (DRY RUN)" : ""}\n`);

if (!existsSync(paths.legacyAgentDir)) {
  console.log("  ~/.pi/agent no existe: nada que migrar (¿ya migrado, o Ein-Pi sin instalar?).");
  process.exit(0);
}
...
try {
  ...
  console.log("\n✓ Migrado. Ahora:");
  console.log("  · ein-pi   → tu EIN (aislado, con tu login y sesiones)");
