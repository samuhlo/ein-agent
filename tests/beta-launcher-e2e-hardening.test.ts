    ...
    try {
      ...
      expect(readEvents(fixture.evidence).filter((event) => event.kind === "executor")).toEqual([{
        ...
        env: {
          ...
          ENGRAM_DATA_DIR: join(fixture.home, ".engram-ein"),
    ...
    try {
      for (const provider of ["pi", "claude"] as const) {
        ...
        expect(calls[0]?.env).toEqual(provider === "pi"
          ? { PI_CODING_AGENT_DIR: fixture.runtimeHome, EIN_PI_AGENT_HOME: fixture.runtimeHome, ENGRAM_DATA_DIR: join(fixture.home, ".engram-ein") }
          : { CLAUDE_CONFIG_DIR: join(fixture.home, ".claude-ein"), ENGRAM_DATA_DIR: join(fixture.home, ".engram-ein"), PATH: `${join(fixture.home, ".claude-ein", "bin")}:${join(fixture.home, "bin")}` });
      }
    } finally {
      fixture.dispose();
    }
