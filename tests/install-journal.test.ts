...
import { isStructurallyValidInstallJournal } from "../installer/src/core/install-journal-shape.ts";
...
describe("install execution journal", () => {
  test("is closed, bounded, deterministic, and rejects malformed or private identity", async () => {
    ...
    const revoked = Proxy.revocable({ ...journal }, {}), revokedEntries = Proxy.revocable([...journal!.entries], {}); revoked.revoke(); revokedEntries.revoke(); const accessor = { ...journal }, entryAccessor = [...journal!.entries]; Object.defineProperty(accessor, "state", { enumerable: true, get: () => "prepared" }); Object.defineProperty(entryAccessor, "0", { enumerable: true, get: () => journal!.entries[0] }); const invalidPending = { ...journal, pendingEntryId: 42 }, invalidRecovery = { ...journal, recoveryCode: "private" }; const malformed: unknown[] = [{ ...journal, extra: true }, { ...journal, transactionId: "PRIVATE" }, { ...journal, planDigest: "PRIVATE" }, { ...journal, state: "prepared" }, { ...journal, state: "complete" }, { ...journal, entries: entryAccessor }, { ...journal, entries: revokedEntries.proxy }, { ...journal, entries: [{ id: "private.entry", status: "pending" }], pendingEntryId: "private.entry" }, invalidPending, invalidRecovery, Object.assign(Object.create({ inherited: true }), journal), new Proxy({ ...journal }, {}), revoked.proxy, accessor];
    expect(isStructurallyValidInstallJournal(invalidPending)).toBe(false); expect(isStructurallyValidInstallJournal(invalidRecovery)).toBe(false);
