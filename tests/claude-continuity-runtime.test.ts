...
if (process.env.EIN_CONTINUITY_ENDPOINT) {
  ...
  process.stdin.on("data", () => process.exit(0)); process.stdin.resume(); process.stdout.write("\\u001b[?2004h");
    ...
    try {
      for (const target of ["pi", "claude"]) { const result = await execute("handoff", target); if (result.code !== 0) throw new Error(result.output); expect(result.output).toContain('SOURCEARGV:["-c","--resume","hook"]'); expect(result.output).toContain(`DEST:${target}:[]:IPC=false`); expect(result.output).toContain(target === "pi" ? `:CC=none:PI=${join(root, "pi-agent")}:ENGRAM=${join(root, ".engram-pi")}` : `:CC=${join(root, "claude-config")}:PI=none:ENGRAM=${join(root, ".engram-cc-ein")}`); expect(result.output).not.toContain("/hostile/source"); expect(result.output).not.toContain("PRIVATE-BRIEF"); }
