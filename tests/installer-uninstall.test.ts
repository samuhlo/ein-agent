...
function seed(): { home: string; binDir: string } {
  ...
  for (const [path, value] of [[".pi-ein/agent/auth.json", "AUTH"], [".pi-ein/agent/sessions/session.json", "SESSION"], [".pi-ein/agent/backups/installer/backup.tar.gz", "BACKUP"], [".pi-ein/agent/skills/user/SKILL.md", "FOREIGN"], [".claude-ein/agents/mine.md", "FOREIGN"], [".claude-ein/commands/ein/mine.md", "FOREIGN"], [".claude-ein/history.jsonl", "HISTORY"], [".claude-ein/sessions/session.json", "SESSION"], [".engram-pi/store", "MEMORY"], [".engram-cc-ein/store", "MEMORY"], [".config/opencode-secrets/token", "SECRET"]] as const) put(path, value);
  return { home, binDir };
}
