...
const toolNames = Object.keys(definitions);
const release = installToolCardBridge(ToolExecutionComponent.prototype, { matches: (name) => toolNames.includes(name), duration: () => 200, render: renderNativeCard });
assert(release);
...
try {
  for (const tool of toolNames) {
    for (const state of ["pending", "running", "completed", "failed", "cancelled", "empty"]) {
      const args = (tool === "bash" || tool === "powershell") ? { command: "env -u DATABASE_URL -u APP_ENV -u DATABASE_ENV bun -e '/* inspect connection without inherited variables */'" }
        : { path: "openspec/changes/demo/tasks.md", edits: [{ oldText: "- [] 2.1 Modelo", newText: "- [x] 2.1 Modelo" }], content: "example", pattern: "active" };
      const view = new ToolExecutionComponent(tool, "fixture", args, {}, definitions[tool as keyof typeof definitions], { requestRender() {} } as any, process.cwd());
      ...
      for (const width of [32, 40, 60, 80, 120]) for (const expanded of [false, true]) {
        ...
        assert.equal(plain.filter((line) => /^[●✓✗■] /.test(line)).length, 1, JSON.stringify({ tool, state, width, expanded, plain }));
  ...
  const image = new ToolExecutionComponent("read", "image", { path: "design.png" }, {}, definitions.read, { requestRender() {} } as any, process.cwd());
...
console.log(`Native cards: ${snapshots.length} real Pi renders passed; ${toolNames.length} builtins, six states, five widths, expansion and Kitty images.`);
